...
rm(list = ls())
setwd('~/Desktop/Repositories/lexidate-variation-analysis/Data-Tools/Visualizer/')
cat("\014")
...
test_plot <- function(task, spliter)
{
  return(filter(classification_scores, task_id == task & split == spliter) %>%
    ggplot(., aes(x = selection, y = testing_performance, color = selection,
...
train_plot <- function(task, spliter)
{
  filter(classification_scores, task_id == task & split == spliter) %>%
    ggplot(., aes(x = selection, y = training_performance, color = selection,
                  fill = selection, shape = selection)) +
    geom_flat_violin(position = position_nudge(x = 0.1, y = 0),
                     scale = "width", alpha = 0.2, width = 1.5) +
    geom_boxplot(color = "black", width = .08, outlier.shape = NA, alpha = 0.0,
...
train_plot_last_col <- function(task, spliter)
{
  filter(classification_scores, task_id == task & split == spliter) %>%
    ggplot(., aes(x = selection, y = training_performance, color = selection,
                  fill = selection, shape = selection)) +
...
train_row_p <- function(train_10, train_20, train_50, train_80, train_90)
{
  plot_grid(
    train_10 + col_theme + theme(plot.title = element_blank()),
    train_20 + col_theme + theme(plot.title = element_blank()),
    train_50 + col_theme + theme(plot.title = element_blank()),
    train_80 + col_theme + theme(plot.title = element_blank()),
    train_90 + theme(legend.position = "none", axis.title.x=element_blank(),
                      axis.ticks.x = element_blank(), axis.text.x = element_blank(), 
                      axis.text.y = element_text(angle = 90, hjust = 0.5),
