...
names(regression_scores)[names(regression_scores) == "testing_performance.lexicase"] <- "lexicase"
names(regression_scores)[names(regression_scores) == "testing_performance.tournament"] <- "tournament"
...
# generate figure
testing_error = plot_grid(
  ggdraw() + draw_label("Average absolute error on test set per OpenML regression task", fontface='bold', size = 20) + p_theme,
  testing_25,
  testing_50,
  testing_75,
  bottom,
...
save_plot(
  paste(filename ="testing_error.pdf"),
  testing_error,
  base_width=10,
  base_height=10
)
