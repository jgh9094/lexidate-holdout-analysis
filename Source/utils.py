...
def lex_selection_objectives(est,X,y,X_select,y_select):
    # fit model
    est.fit(X,y)
...
def aggregated_selection_objectives(est,X,y,X_select,y_select):
    # fit model
    est.fit(X,y)
...
def get_estimator_params(n_jobs,
    ...
    if scheme == 'lexicase':
        ...
        objective_scorer = partial(lex_selection_objectives,X=X_learn,y=y_learn,X_select=X_select,y_select=y_select)
        ...
        # create selection objective functions
        objective_scorer = partial(aggregated_selection_objectives,X=X_learn,y=y_learn,X_select=X_select,y_select=y_select)
...
def execute_experiment(split_select, scheme, task_id, n_jobs, save_path, seed):
    ...
    try:
        ...
        names, est_params = get_estimator_params(n_jobs=n_jobs,scheme=scheme,split_select=split_select,X_train=X_train,y_train=y_train,seed=seed)
