# Task 359962

We present the results of our analysis of `task 359962` with the different selection set splits used in our study.

```{r task-359962-data, echo=TRUE}
task_data <- filter(results, task_id == 359962)
```

## 5%

### Test accuracy

```{r task-359962-test-05-perf, echo=TRUE, out.width="100%"}
test_plot(filter(task_data, split == '5%'))
```

Summary statistics for the testing performance of the selection schemes at the 5% selection set split:
```{r task-359962-test-05-sum, echo=TRUE, out.width="100%"}
test_results_summary(filter(task_data, split == '5%'))
```

The permutation test revealed that the results are:
```{r task-359962-test-05-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '5%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '5%' & selection == 'lexicase')
permutation_test(tournament_results$testing_performance,
                    lexicase_results$testing_performance,
                    seed = 121,
                    alternative = "t")
```

### Validation accuracy

```{r task-359962-select-05-perf, echo=TRUE, out.width="100%"}
validation_plot(filter(task_data, split == '5%'))
```

Summary statistics for the testing performance of the selection schemes at the 5% selection set split:
```{r task-359962-select-05-sum, echo=TRUE, out.width="100%"}
validation_accuracy_summary(filter(task_data, split == '5%'))
```

The permutation test revealed that the results are:
```{r task-359962-select-05-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '5%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '5%' & selection == 'lexicase')
permutation_test(tournament_results$training_performance,
                    lexicase_results$training_performance,
                    seed = 122,
                    alternative = "l")
```

### Pipeline complexity

```{r task-359962-complexity-05-perf, echo=TRUE, out.width="100%"}
complexity_plot(filter(task_data, split == '5%'))
```

Summary statistics for the pipeline complexity of the selection schemes at the 5% selection set split:
```{r task-359962-complexity-05-sum, echo=TRUE, out.width="100%"}
complexity_summary(filter(task_data, split == '5%'))
```

The permutation test revealed that the results are:
```{r task-359962-complexity-05-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '5%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '5%' & selection == 'lexicase')
permutation_test(tournament_results$testing_complexity,
                    lexicase_results$testing_complexity,
                    seed = 248,
                    alternative = "t")
```

## 10%

### Test accuracy

```{r task-359962-test-10-perf, echo=TRUE, out.width="100%"}
test_plot(filter(task_data, split == '10%'))
```

Summary statistics for the testing performance of the selection schemes at the 5% selection set split:
```{r task-359962-test-10-sum, echo=TRUE, out.width="100%"}
test_results_summary(filter(task_data, split == '10%'))
```

The permutation test revealed that the results are:
```{r task-359962-test-10-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '10%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '10%' & selection == 'lexicase')
permutation_test(tournament_results$testing_performance,
                    lexicase_results$testing_performance,
                    seed = 123,
                    alternative = "g")
```

### Validation accuracy

```{r task-359962-select-10-perf, echo=TRUE, out.width="100%"}
validation_plot(filter(task_data, split == '10%'))
```

Summary statistics for the testing performance of the selection schemes at the 5% selection set split:
```{r task-359962-select-10-sum, echo=TRUE, out.width="100%"}
validation_accuracy_summary(filter(task_data, split == '10%'))
```

The permutation test revealed that the results are:
```{r task-359962-select-10-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '10%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '10%' & selection == 'lexicase')
permutation_test(tournament_results$training_performance,
                    lexicase_results$training_performance,
                    seed = 124,
                    alternative = "l")
```

### Pipeline complexity

```{r task-359962-complexity-10-perf, echo=TRUE, out.width="100%"}
complexity_plot(filter(task_data, split == '10%'))
```

Summary statistics for the pipeline complexity of the selection schemes at the 5% selection set split:
```{r task-359962-complexity-10-sum, echo=TRUE, out.width="100%"}
complexity_summary(filter(task_data, split == '10%'))
```

The permutation test revealed that the results are:
```{r task-359962-complexity-10-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '10%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '10%' & selection == 'lexicase')
permutation_test(tournament_results$testing_complexity,
                    lexicase_results$testing_complexity,
                    seed = 249,
                    alternative = "t")
```

## 50%

### Test accuracy

```{r task-359962-test-50-perf, echo=TRUE, out.width="100%"}
test_plot(filter(task_data, split == '50%'))
```

Summary statistics for the testing performance of the selection schemes at the 5% selection set split:
```{r task-359962-test-50-sum, echo=TRUE, out.width="100%"}
test_results_summary(filter(task_data, split == '50%'))
```

The permutation test revealed that the results are:
```{r task-359962-test-50-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '50%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '50%' & selection == 'lexicase')
permutation_test(tournament_results$testing_performance,
                    lexicase_results$testing_performance,
                    seed = 125,
                    alternative = "t")
```

### Validation accuracy

```{r task-359962-select-50-perf, echo=TRUE, out.width="100%"}
validation_plot(filter(task_data, split == '50%'))
```

Summary statistics for the testing performance of the selection schemes at the 5% selection set split:
```{r task-359962-select-50-sum, echo=TRUE, out.width="100%"}
validation_accuracy_summary(filter(task_data, split == '50%'))
```

The permutation test revealed that the results are:
```{r task-359962-select-50-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '50%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '50%' & selection == 'lexicase')
permutation_test(tournament_results$training_performance,
                    lexicase_results$training_performance,
                    seed = 126,
                    alternative = "t")
```

### Pipeline complexity

```{r task-359962-complexity-50-perf, echo=TRUE, out.width="100%"}
complexity_plot(filter(task_data, split == '50%'))
```

Summary statistics for the pipeline complexity of the selection schemes at the 5% selection set split:
```{r task-359962-complexity-50-sum, echo=TRUE, out.width="100%"}
complexity_summary(filter(task_data, split == '50%'))
```

The permutation test revealed that the results are:
```{r task-359962-complexity-50-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '50%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '50%' & selection == 'lexicase')
permutation_test(tournament_results$testing_complexity,
                    lexicase_results$testing_complexity,
                    seed = 250,
                    alternative = "t")
```

## 90%

### Test accuracy

```{r task-359962-test-90-perf, echo=TRUE, out.width="100%"}
test_plot(filter(task_data, split == '90%'))
```

Summary statistics for the testing performance of the selection schemes at the 5% selection set split:
```{r task-359962-test-90-sum, echo=TRUE, out.width="100%"}
test_results_summary(filter(task_data, split == '90%'))
```

The permutation test revealed that the results are:
```{r task-359962-test-90-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '90%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '90%' & selection == 'lexicase')
permutation_test(tournament_results$testing_performance,
                    lexicase_results$testing_performance,
                    seed = 127,
                    alternative = "t")
```

### Validation accuracy

```{r task-359962-select-90-perf, echo=TRUE, out.width="100%"}
validation_plot(filter(task_data, split == '90%'))
```

Summary statistics for the testing performance of the selection schemes at the 5% selection set split:
```{r task-359962-select-90-sum, echo=TRUE, out.width="100%"}
validation_accuracy_summary(filter(task_data, split == '90%'))
```

The permutation test revealed that the results are:
```{r task-359962-select-90-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '90%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '90%' & selection == 'lexicase')
permutation_test(tournament_results$training_performance,
                    lexicase_results$training_performance,
                    seed = 128,
                    alternative = "t")
```

### Pipeline complexity

```{r task-359962-complexity-90-perf, echo=TRUE, out.width="100%"}
complexity_plot(filter(task_data, split == '90%'))
```

Summary statistics for the pipeline complexity of the selection schemes at the 5% selection set split:
```{r task-359962-complexity-90-sum, echo=TRUE, out.width="100%"}
complexity_summary(filter(task_data, split == '90%'))
```

The permutation test revealed that the results are:
```{r task-359962-complexity-90-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '90%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '90%' & selection == 'lexicase')
permutation_test(tournament_results$testing_complexity,
                    lexicase_results$testing_complexity,
                    seed = 251,
                    alternative = "t")
```

## 95%

### Test accuracy

```{r task-359962-test-95-perf, echo=TRUE, out.width="100%"}
test_plot(filter(task_data, split == '95%'))
```

Summary statistics for the testing performance of the selection schemes at the 5% selection set split:
```{r task-359962-test-95-sum, echo=TRUE, out.width="100%"}
test_results_summary(filter(task_data, split == '95%'))
```

The permutation test revealed that the results are:
```{r task-359962-test-95-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '95%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '95%' & selection == 'lexicase')
permutation_test(tournament_results$testing_performance,
                    lexicase_results$testing_performance,
                    seed = 129,
                    alternative = "t")
```

### Validation accuracy

```{r task-359962-select-95-perf, echo=TRUE, out.width="100%"}
validation_plot(filter(task_data, split == '95%'))
```

Summary statistics for the testing performance of the selection schemes at the 5% selection set split:
```{r task-359962-select-95-sum, echo=TRUE, out.width="100%"}
validation_accuracy_summary(filter(task_data, split == '95%'))
```

The permutation test revealed that the results are:
```{r task-359962-select-95-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '95%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '95%' & selection == 'lexicase')
permutation_test(tournament_results$training_performance,
                    lexicase_results$training_performance,
                    seed = 130,
                    alternative = "t")
```

### Pipeline complexity

```{r task-359962-complexity-95-perf, echo=TRUE, out.width="100%"}
complexity_plot(filter(task_data, split == '95%'))
```

Summary statistics for the pipeline complexity of the selection schemes at the 5% selection set split:
```{r task-359962-complexity-95-sum, echo=TRUE, out.width="100%"}
complexity_summary(filter(task_data, split == '95%'))
```

The permutation test revealed that the results are:
```{r task-359962-complexity-95-pv, echo=TRUE, out.width="100%"}
tournament_results <- filter(task_data, split == '95%' & selection == 'tournament')
lexicase_results <- filter(task_data, split == '95%' & selection == 'lexicase')
permutation_test(tournament_results$testing_complexity,
                    lexicase_results$testing_complexity,
                    seed = 252,
                    alternative = "t")
```